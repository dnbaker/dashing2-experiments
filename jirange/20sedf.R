...
capture.output(print(tb, n=nrow(tb), width=200), file=paste0('20sedf_ani_sae.tbl'))
...
tb <- se_df(ani=F, bins=1, include_types=incl_types_ji) %>%
  select(-se, -sse, -bin) %>%
  pivot_wider(names_from='type', values_from='sae')
capture.output(print(tb, n=nrow(tb), width=200), file=paste0('20sedf_ji_sae.tbl'))
...
tb <- se_df(ani=T, bins=20, include_types=incl_types_ani) %>%
  select(-se, -sse) %>%
  pivot_wider(names_from='type', values_from='sae')
capture.output(print(tb, n=nrow(tb), width=200), file=paste0('20sedf_ani_bins_sae.tbl'))
...
tb <- se_df(ani=F, bins=20, include_types=incl_types_ji) %>%
  select(-se, -sse) %>%
  pivot_wider(names_from='type', values_from='sae')
capture.output(print(tb, n=nrow(tb), width=200), file=paste0('20sedf_ji_bins_sae.tbl'))
