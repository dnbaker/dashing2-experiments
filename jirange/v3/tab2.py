...
expected_columns = ['bin', 'k', 'sketchsize', 'n', 'BD1', 'BD8', 'Dash1', 'FSS1', 'FSS8', 'Mash', 'SM', 'SS1', 'SS8']
use_sketches = ['8192', '32768', '131072']
use_tools = ['Mash', 'SM', 'BD1', 'Dash1', 'FSS1', 'SS1']
use_ks = ['21', '31']  # don't have 31 it seems
...
print('\\begin{table}')
print('\\begin{tabular}{rr' + ('r' * len(use_tools)) + '}')
print(' & '.join(['k', 'kbits'] + tools_renamed) + ' \\\\ \\hline')
last_k = ''

# The .fixed version fixes some rows that were split in two because of one bad run presumably
with open('20sedf_ji_sse_subset.tbl', 'rb') as fh:
    for ln in fh:
        ln = ln.rstrip()
...
print('\\end{tabular}')
print('\\caption{Sum of squared error JI for ANI >= 89}')
print('\\end{table}')
